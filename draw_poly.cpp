...
int num_sides; //number of sides for the polygon
double side_length=0.5; //Length of each polygon side (or circle radius)
double Radius; //radius of a circle if num_sides>10
...
void Turnprog(const tf::tfMessage cvalue)
{
  ...
  {
    target_angle =  current_angle - PI*2/num_sides;
  }
...
void circleprog(const tf::tfMessage cvalue)
{
  double turnz, turnw, mindist;
  ...
  if(target_angle == INITIALIZE_VALUE)
  {
    target_angle = current_angle+PI/15;
  }
...
int main(int argc, char **argv)
{
  ...
  //prompt the user for an input for # of sides
  std::cout << "Enter the number of sides for polygon: ";
  //get the input from the user
  std::cin >> num_sides;
  
  //prompt for side length
  std::cout << "Enter the side length of polygon: ";
  //get the input from the user
  std::cin >> target_forward;
  ...
  if (num_sides <= 10) //POLYGON
	{
	  // LOOPS N TIMES  (FOR EACH SIDE OF POLYGON)
	  for(int i=0;i<num_sides;i++)
	  {
	    ...
	    while(flag && ros::ok())
	    {
	      //PUBLISHES THE TARGET SPEED, CALCULATED WHEN THE ROBOT POSITION IS SUBSCRIBED
	    ...
	    target_angle=target_angle-PI*2/num_sides;
  ...
  else   //circle code
	{
	    ...
	    // DECLARE SUBSCRIBER TO THE TF TOPIC (MR POSE) WITH THE FUNCTION TURNPROG
	    tf = n.subscribe("/tf", 1, circleprog);
	    ros::spinOnce();
	    loop_rate.sleep();
	    
	    // SPINS COUNTER CLOCKWISE UNTIL THE DESIRED ANGLE IS REACHED
	    while(flag && ros::ok())
	    {
		  //PUBLISHES THE TARGET SPEED, CALCULATED when POSITION IS SUBSCRIBED
	      c.linear.x=0.5;
	      c.angular.z=-(c.linear.x/target_forward);
	      vel_pub.publish(c);
